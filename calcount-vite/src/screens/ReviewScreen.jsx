...
function calcTotals(scanned, trackMode, servings, grams) {
  const n = scanned;
  if (trackMode === 'grams') {
    const g = parseFloat(grams) || 0;
    const p100 = n.per100g;
    const hasP100 = p100 && p100.calories != null;
    const base = hasP100 ? p100 : (n.servingGrams ? {
    ...
    if (!base) return { cal: Math.round(n.calories), protein: n.protein, carbs: n.carbs, fat: n.fat, fiber: n.fiber };
    const factor = g / 100;
    return {
...
export default function ReviewScreen() {
  ...
  function handleAddEntry() {
    if (!foodName.trim()) { toast('Enter a food name', 'error'); return; }
